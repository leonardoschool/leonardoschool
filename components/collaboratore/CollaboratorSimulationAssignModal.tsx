...
interface CollaboratorSimulationAssignModalProps {
  isOpen: boolean;
  onClose: () => void;
  simulationId: string;
...
/**
 * Assignment modal for collaborators
 * Collaborators can only assign to their groups and students within those groups
 * They cannot assign to classes
 */
export function CollaboratorSimulationAssignModal({
  isOpen,
  onClose,
  simulationId,
  simulationTitle,
  onSuccess,
}: CollaboratorSimulationAssignModalProps) {
  ...
  // Fetch data - collaborators only see their own groups and students
  const { data: groupsData } = trpc.groups.getGroups.useQuery(
    { page: 1, pageSize: 100 },
    { enabled: isOpen }
  );
  const { data: studentsData } = trpc.students.getFromMyGroups.useQuery(
    undefined,
    { enabled: isOpen }
  );
      ...
      >
        ...
        <div className="px-6 py-4 overflow-y-auto max-h-[calc(90vh-140px)] space-y-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className={`text-sm text-blue-700 dark:text-blue-300`}>
                ...
                <p className="mt-1 opacity-80">
                  Quando assegni la simulazione a studenti o gruppi, verrà automaticamente pubblicata
          ...
          <div className="space-y-4">
            ...
            {/* Students from my groups */}
            {students.length > 0 && (
              <div>
                <label className={`block text-sm font-medium ${colors.text.secondary} mb-2`}>
                  Studenti dei tuoi gruppi
                </label>
            ...
            {groups.length === 0 && students.length === 0 && (
              <div className={`text-center py-8 ${colors.text.muted}`}>
                ...
                <p className="text-sm mt-1">Assicurati di avere gruppi assegnati</p>
              </div>
            )}
          </div>
...
export default CollaboratorSimulationAssignModal;
