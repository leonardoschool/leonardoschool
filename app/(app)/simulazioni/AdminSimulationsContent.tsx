...
export default function AdminSimulationsContent() {
  ...
  const duplicateMutation = trpc.simulations.duplicate.useMutation({
    onSuccess: async (duplicated) => {
      showSuccess('Duplicata', 'Simulazione duplicata come bozza. Puoi convertirla in esercitazione dalla pagina di modifica.');
      await utils.simulations.getSimulations.invalidate();
      router.push(`/simulazioni/${duplicated.id}/modifica`);
    },
          ...
          >
            ...
            <div className="px-3 sm:px-2 py-2">
              ...
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 sm:px-3 py-3 sm:py-2.5 text-sm sm:text-sm rounded-lg ${colors.text.primary} hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors touch-manipulation disabled:opacity-50`}
              >
        ...
        return (
          <Portal>
            ...
            <div
              ...
              style={{
                top: typeof window !== 'undefined' && window.innerWidth >= 640 ? assignmentMenuPosition.top : undefined,
                left: typeof window !== 'undefined' && window.innerWidth >= 640 ? assignmentMenuPosition.left : undefined,
              }}
            >
              <div className="sm:hidden flex justify-center py-2">
                <div className="w-12 h-1 rounded-full bg-gray-300 dark:bg-gray-600" />
              ...
              {hasNavItems && (
                <>
                  <div className="px-3 sm:px-2 pb-2 pt-2">
                    ...
                    {activeGroup.simulationAccessType === 'ROOM' && activeGroup.status === 'ACTIVE' && (
                      isExpired ? (
                        ...
                        <button
                          onClick={() => {
                            ...
                            window.open(url, `VirtualRoom_${activeGroup.id}`, `width=${width},height=${height},left=${left},top=${top},menubar=no,toolbar=no,location=no,status=no,scrollbars=yes,resizable=yes`);
                            setOpenAssignmentMenuId(null);
                          }}
