...
export default function TagsManagementPage() {
  ...
  const openEditCategory = (category: { id?: string; name?: string; description?: string | null; color?: string | null; order?: number }) => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className={`mt-1 ${colors.text.secondary}`}>
            Gestisci categorie e tag per organizzare le domande
          </p>
        </div>
          ...
          <div className="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            {uncategorizedTags.length > 0 && (
              <div className={`border-t-2 ${colors.border.primary}`}>
                ...
                <div
                  className="p-4 flex items-center gap-4 bg-gray-50 dark:bg-gray-800/30"
                >
                  <div className="w-6" /> {/* Spacer for alignment */}
                  
                  <div
                    className="w-4 h-4 rounded-full flex-shrink-0 bg-gray-400 dark:bg-gray-600"
                  />
