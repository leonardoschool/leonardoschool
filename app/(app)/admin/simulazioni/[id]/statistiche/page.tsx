...
export default function SimulationStatsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: questionAnalysis, isLoading: analysisLoading } = trpc.simulations.getQuestionAnalysis.useQuery(
    { simulationId: id },
    { enabled: !!simulation }
  );
  ...
  return (
    <div className="p-6">
      ...
      {/* Tabs */}
      <div className="mb-6">
        <div className={`flex gap-1 p-1 rounded-lg ${colors.background.card} border ${colors.border.light} w-fit`}>
          <button
      ...
      {activeTab === 'overview' && (
        <>
          ...
          {questionAnalysis && questionAnalysis.subjectBreakdown.length > 0 && (
            <div className={`mb-8 rounded-xl ${colors.background.card} border ${colors.border.light} overflow-hidden`}>
