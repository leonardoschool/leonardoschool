...
export default function LeaderboardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getRankBadge = (rank: number) => {
    switch (rank) {
      case 1:
        return (
          <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600 shadow-lg">
            <Crown className="w-5 h-5 text-white" />
          </div>
        );
      case 2:
        return (
          <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-gray-300 to-gray-500 shadow-lg">
            <Medal className="w-5 h-5 text-white" />
          </div>
        );
      case 3:
        return (
          <div className="flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br from-amber-600 to-amber-800 shadow-lg">
            <Medal className="w-5 h-5 text-white" />
          </div>
        );
      default:
        return (
          <div className={`flex items-center justify-center w-10 h-10 rounded-full ${colors.background.secondary}`}>
            <span className={`text-lg font-bold ${colors.text.primary}`}>{rank}</span>
          </div>
        );
    }
  };
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 max-w-5xl mx-auto">
      ...
      <div className="mb-8">
        <Link
          href={`/admin/simulazioni/${id}/statistiche`}
      ...
      {leaderboard.length >= 3 && (
        <div className="mb-8">
          <div className="grid grid-cols-3 gap-4 items-end">
            {/* Second Place */}
            <div className={`text-center p-4 rounded-xl ${colors.background.card} border ${colors.border.light}`}>
              <div className="flex items-center justify-center w-12 h-12 mx-auto mb-3 rounded-full bg-gradient-to-br from-gray-300 to-gray-500 shadow-lg">
                <Medal className="w-6 h-6 text-white" />
              </div>
              <p className={`font-semibold ${colors.text.primary} truncate`}>{leaderboard[1].studentName}</p>
            ...
            {/* First Place */}
            <div className={`text-center p-6 rounded-xl ${colors.background.card} border-2 border-yellow-400 shadow-lg transform scale-105`}>
              <div className="flex items-center justify-center w-16 h-16 mx-auto mb-3 rounded-full bg-gradient-to-br from-yellow-400 to-yellow-600 shadow-xl">
                <Crown className="w-8 h-8 text-white" />
              </div>
              <p className={`font-bold text-lg ${colors.text.primary} truncate`}>{leaderboard[0].studentName}</p>
            ...
            {/* Third Place */}
            <div className={`text-center p-4 rounded-xl ${colors.background.card} border ${colors.border.light}`}>
              <div className="flex items-center justify-center w-12 h-12 mx-auto mb-3 rounded-full bg-gradient-to-br from-amber-600 to-amber-800 shadow-lg">
                <Medal className="w-6 h-6 text-white" />
              </div>
              <p className={`font-semibold ${colors.text.primary} truncate`}>{leaderboard[2].studentName}</p>
