...
export const materialsRouter = router({
  ...
  getStats: staffProcedure.query(async ({ ctx }) => {
    const [totalMaterials, totalByType, totalByVisibility, topViewed, topDownloaded] = await Promise.all([
      ...
      ctx.prisma.material.groupBy({ by: ['visibility'], _count: true, where: { isActive: true } }),
    ...
    return {
      totalMaterials,
      byType: totalByType.reduce((acc, item) => ({ ...acc, [item.type]: item._count }), {} as Record<string, number>),
      byVisibility: totalByVisibility.reduce((acc, item) => ({ ...acc, [item.visibility]: item._count }), {} as Record<string, number>),
      topViewed,
      topDownloaded,
    };
  }),
